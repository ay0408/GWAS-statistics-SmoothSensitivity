...
${\ \ \ }$ ← I expect that it is not so difficult to obtain a more rigorous $(\alpha, \beta)$ ${admissible}$ property of the general distribution with density $h(z) \propto \frac{1}{1+|z|^\gamma}$ (than was previously reported in [Smooth Sensitivity and Sampling in Private Data Analysis](https://cs-people.bu.edu/ads22/pubs/NRS07/NRS07-full-draft-v1.pdf)) by following our Theorem 1 and its proof. However, for a quantitative evaluation and comparison among noise distributions, I believe that it is more important to have a closer discussion on inequalities in the proof and accordingly extend and elaborate the ${admissible}$ property and concept of ${smooth\ sensitivity}$.
...
・The last sentence in Section III.B.1. should be ${\it deleted}$ or rewritten as "The global sensitivities analyzed in existing studies were for cases where the number of cases and controls are fixed, and this study is the first to consider cases without such limitations. In particular, we analyzed them approximately where the number of cases and controls are almost equal and not fixed." (Theorems 5 and 6 can be used for any cases without restrictions on the number of cases and controls, but the evaluation and discussion are still insufficient as a whole.)
